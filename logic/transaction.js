...
Transaction.prototype.create = function (data) {
	...
	if(trs.type==0){  //转账
		var fee = getCalculate(trs.amount);
		trs.fee = fee;
	}else{   //其他交易
		trs.fee = privated.types[trs.type].calculateFee.call(this, trs, data.sender) || false;
	}
...
function getCalculate(amount){
	var amountFee = amount*0.001;
	if(amountFee>=5000000000){
		amountFee = 5000000000;
	}
	return amountFee;
}
...
Transaction.prototype.verify = function (trs, sender, requester, cb) { //inheritance
	...
	// Check amount
	if (trs.amount < 0 || trs.amount > 1000000000 * constants.fixedPoint || String(trs.amount).indexOf('.') >= 0 || trs.amount.toString().indexOf('e') >= 0) {
...
Transaction.prototype.apply = function (trs, block, sender, cb) {
	...
	if (trs.blockId != genesisblock.block.id && sender.balance < amount) {
		return setImmediate(cb, "Account has no Coin: " + trs.id);
	}
